...
export async function upsertProfile(data) {
  ...
  if (existing) {
    await pool.query(
      `UPDATE Roommate_Profile SET
         bio = ?, sleep_schedule = ?, cleanliness_level = ?, noise_tolerance = ?,
         study_habits = ?, social_level = ?, smoking = ?, pets = ?,
         budget_min = ?, budget_max = ?, move_in_date = ?
       WHERE room_profile_id = ?`,
      [
        data.bio ?? null, data.sleepSchedule, data.cleanlinessLevel, data.noiseTolerance,
        data.studyHabits, data.socialLevel, data.smoking, data.pets,
        data.budgetMin ?? null, data.budgetMax ?? null, data.moveInDate ?? null,
  ...
  const [result] = await pool.query(
    `INSERT INTO Roommate_Profile
       (user_id, school_id, semester, semester_year, bio, sleep_schedule, cleanliness_level,
        noise_tolerance, study_habits, social_level, smoking, pets, budget_min, budget_max, move_in_date)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      data.userId, data.schoolId, data.semester, data.semesterYear, data.bio ?? null,
      data.sleepSchedule, data.cleanlinessLevel, data.noiseTolerance, data.studyHabits,
      data.socialLevel, data.smoking, data.pets, data.budgetMin ?? null, data.budgetMax ?? null,
